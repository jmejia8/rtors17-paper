...
mechanics, which will be called Evolutionary Centers Algorithm (ECA). We utilize the center of mass definition for creating new directions for moving the worst elements in the population, %
based on their objective function values %
 to better regions. %
 % respect to the objective function. It will be seen that using this strategy will be fortuitous.
 The efficiency of the new approach is demonstrated using the CEC17 benchmark functions. We present a comparison with the best algorithm (jSO) in the  CEC17 competition. 
%
...
% they are quite simple to implement in any language. Exists two branch of population based algorithms: 
